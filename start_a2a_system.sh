...
# Require the caller to activate the desired environment with `so`.
# This script now uses the active virtualenv instead of guessing a different one.
if [ -z "${VIRTUAL_ENV:-}" ] || [ ! -x "$VIRTUAL_ENV/bin/python" ]; then
    echo "Activate the desired environment with 'so' before running this script." >&2
    exit 1
fi
...
# ========================================
log_info "Step 1/4: *** SKIPPING *** Starting Letta Server (Unified Memory Backend)..."
log_info "*** Letta Server running on a separate system.  Not starting here... ***"
# we are running letta server on a separate system for now. jan 2026
...
nohup "$PYTHON_BIN" -u main.py > "$LOG_DIR/orchestrator.log" 2>&1 &
ORCHESTRATOR_PID=$!
...
nohup "$PYTHON_BIN" -u main.py > "$LOG_DIR/dashboard_agent.log" 2>&1 &
DASHBOARD_AGENT_PID=$!
