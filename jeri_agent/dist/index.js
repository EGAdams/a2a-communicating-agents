...
const TARGET_DIR = process.env.JERI_TARGET_DIR ??
    '/home/adamsl/rol_finances/readable_documents/bank_statements/january';
const ARTIFACT_ROOT = process.env.JERI_ARTIFACT_ROOT ?? '/home/adamsl/rol_finances/artifacts';
function log(msg) {
    console.log(`[${AGENT_NAME}] ${msg}`);
}
function scopeAllowed(requestedPath) {
...
function inferCapability(description, explicit) {
    if (explicit)
        return explicit;
    const d = description.toLowerCase();
    if (d.includes('anomal'))
        return 'find_anomalies';
    if (d.includes('extract'))
        return 'extract_transactions';
    if (d.includes('snippet') || d.includes('report'))
        return 'prepare_report_snippet';
    if (d.includes('summary') || d.includes('summar'))
        return 'summarize_statement';
    return 'analyze_statement';
}
async function buildCapabilityResult(capability, description, context = {}) {
        ...
        default:
            return {
                ...
                details: {
                    ...
                    recommendation: manual.length > 0
...
async function handleInbound(ws, msg) {
    if (msg.type !== 'message')
        return;
    if (!msg.content || !msg.topic)
        return;
    if (msg.topic !== AGENT_TOPIC)
        return;
    if (msg.from_agent?.toLowerCase() === AGENT_NAME)
        return;
    let request = null;
    try {
