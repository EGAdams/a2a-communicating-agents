...
class A2ACollectiveHub:
    ...
    async def _create_memory_for_agent(self, agent_id: str) -> Tuple[str, MemoryBackend]:
        ...
        kwargs = {}
        if self.document_manager_cls is not None:
            try:
                kwargs["doc_manager"] = self.document_manager_cls()
            except TypeError:
                kwargs["doc_manager"] = self.document_manager_cls
        return await factory.create_memory_async(agent_id=agent_id, **kwargs)
