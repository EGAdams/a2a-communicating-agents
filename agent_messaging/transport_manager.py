...
class TransportManager:
    ...
    async def _ensure_lock(cls):
        """Ensure lock exists and belongs to the current event loop."""
        # asyncio.Lock is bound to the running event loop. When post_message()
        # is called via asyncio.run(), a fresh loop is created each time, so
        # the cached lock from a previous run() call is invalid. Always recreate.
        try:
            if cls._lock is not None:
                # Probe the lock — raises RuntimeError if it belongs to a dead loop
                cls._lock.locked()
        except RuntimeError:
            cls._lock = None
    ...
    async def get_transport(
        ...
        **kwargs
        ...
        async with cls._lock:
            ...
            # but only if the transport is still healthy. When post_message() is
            # called via asyncio.run(), a new event loop is created each time and
            # the old receiver_task/websocket become dead. Detect that here.
            if cls._initialized and not force_reconnect:
                transport = cls._transport
                healthy = True
                if hasattr(transport, '_receiver_task'):
                    receiver_alive = (
                        transport._receiver_task is not None
                        and not transport._receiver_task.done()
                    )
                    ws_open = (
                        transport._websocket is not None
                        and not getattr(transport._websocket, 'closed', True)
                    )
                    if not receiver_alive or not ws_open:
                        logger.warning(
                            f"Shared transport is stale "
                            f"(receiver_alive={receiver_alive}, ws_open={ws_open}), reconnecting..."
                        )
                        healthy = False
                if healthy:
                    logger.debug(
