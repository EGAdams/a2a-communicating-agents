...
class WebSocketTransport(MessageTransport):
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            # ping_interval/ping_timeout match the server's settings so that
            # brief pauses in processing don't trigger a keepalive disconnect.
            self._websocket = await websockets.connect(
                self.config.url,
                max_size=10 * 1024 * 1024,
                ping_interval=30,   # match server ping_interval
                ping_timeout=60,    # match server ping_timeout
            )
    ...
    async def send(self, message: AgentMessage) -> bool:
        ...
        # Real WebSocket send
        try:
            # Drain any leftover ACKs from a previous timed-out send so they
            # don't cause the new send() to return prematurely or block.
