...
async def main():
    ...
    # Configure websockets with increased ping intervals and message size limit
    async with websockets.serve(
        handler,
        host,
        port,
        ping_interval=30,      # Send ping every 30 seconds (default: 20)
        ping_timeout=60,       # Wait 60 seconds for pong (default: 20)
        max_size=10 * 1024 * 1024,  # 10MB max message size (default: 1MB)
    ):
